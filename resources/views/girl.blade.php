<!DOCTYPE html>
<html lang="ja">
<head>
  <meta charset="UTF-8">
  <meta http-equiv="X-UA-Compatible" content="IE=edge">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <link rel="stylesheet" href="css/reset.css">
    <link rel="stylesheet" href="css/style.css">
  <title>名古屋栄メンズエステ【Alice（アリス）】</title>
</head>
<body>

<div id="mobileback">
<div class="pc">

<header class="header">
        <p class="htext">お得なイベント開催中！</p>
</header>

<div class="fixfooter">
<div class="foot-btn">

    <a href="tel:080-4590-2442"  class=”nurnur”>
  
        <p class="tell-btm">

          <span class="tell-btm2">受付時間24時間</span>
          </br>今すぐ電話予約</p>
          </a>
          
<div class="rec-btm2">
  <a href="/form">
        <p class="rec-btm">セラピスト求人は</br>
      こちらをクリック</p></a></div>

</div>
</div>


<input type="checkbox" id="drawer">
<label for="drawer" class="open"><span></span></label>
<label for="drawer" class="close"></label>
<nav class="menu">
	<h2>menu</h2>
	<ul>

  		<li>
    <a href="/" >トップページ</a>

  </li>
		<li>
    <a href="list" >セラピスト一覧</a>

  </li>
		<li>
    <a href="/schedule">本日のセラピスト</a>
  </li>
		<li> 
    <a href="/menu">メニュー</a>
  </li>
		<li><a href="/access">アクセス</a></li>
	</ul>
</nav>



<div>
    <div class="tophome">
            名古屋栄メンズエステ【Alice（アリス）】
        </div>
        <div class="br2">
            <img src="img/IMG_1885.JPG" alt="">
        </div>
       
        <div class="line">
      </div>
</div>

  <div id="back">
    <div class="topgirl">
            <img src="img/IMG_1618.JPG" alt="">
        </div>
  <h1 class="name">{{$form['name']}}（20歳）</h1>
  <h2 class="girl_title ">身長</h2>
  <p class="girl_title2">**cm</p>
    <h2 class="girl_title ">血液型</h2>
  <p class="girl_title2">**型</p>
      <h2 class="girl_title ">得意なマッサージ</h2>
  <p class="girl_title2">❤︎</p>
        <h2 class="girl_title ">お店からのメッセージ</h2>
  <p class="girl_title2">❤︎</p>

    <div class="tweet">
    <a class="twitter-timeline" href="https://twitter.com/Aman_esute_mens?ref_src=twsrc%5Etfw" data-width="100%" data-tweet-limit="1" data-chrome="noheader">Tweets by cancan_2022</a> <script async src="https://platform.twitter.com/widgets.js" charset="utf-8"></script>
</div>

<table class="table_main" cellpadding="15">
        <tr class="table_col">
          <th>11/13</br>(日)</th>
            <th>11/14</br>(月)</th>
            <th>11/15</br>(火)</th>
            <th>11/16</br>(水)</th>
            <th>11/17</br>(木)</th>
            <th>11/18</br>(金)</th>
            <th>11/19</br>(土)</th>
        </tr>
        <tr>
            <td>12:00〜</br>
          21:00</td>
            <td>12:00〜</br>
          21:00</td>
          <td>-</td>
            <td>12:00〜</br>
          21:00</td>
          <td>-</td>
            <td>-</td>
          <td>12:00〜</br>
          21:00</td>
        </tr>
    </table>

<div class="space">
    <p class="common__ttl">Pickup Theraphist</p>
    <div class="flex__item">
      <a class="item" href="">
        <img src="img/IMG_1618.JPG" alt="">
</a>
<a class="item" href="">
        <img src="img/IMG_1618.JPG" alt="">
</a>
<a class="item" href="">
        <img src="img/IMG_1618.JPG" alt="">
</a>
</div>
<div class="flex__item">
      <a class="item" href="">
        <img src="img/IMG_1618.JPG" alt="">
</a>
<a class="item" href="">
        <img src="img/IMG_1618.JPG" alt="">
</a>
<a class="item" href="">
        <img src="img/IMG_1618.JPG" alt="">
</a>
</div>
</div>




<div class="space">
    <p class="common__ttl">Today's Theraphist</p>
    
</div>
</div>
<footer class="footer">
        <p>Copyright © 名古屋栄メンズエステ【Alice（アリス）】 All Rights Reserved.</p>
    </footer>

        </div>
</div>